...
#include "util/nms.h"
#include "util/hex.h"
#include "model/detect_model_encoded.h"

namespace seeta {
namespace fd {

bool FuStDetector::LoadModel() {
  const std::vector<unsigned char> detect_model_vec = seeta::fd::get_serialized_detect_model_string();
  std::string detect_model_string = bell::hex_to_byte(detect_model_vec);
  std::istringstream model_string(detect_model_string); 
  ...
  model_string.read(reinterpret_cast<char*>(&num_hierarchy_), sizeof(int32_t));
  for (int32_t i = 0; is_loaded && i < num_hierarchy_; i++) {
    model_string.read(reinterpret_cast<char*>(&hierarchy_size),
      sizeof(int32_t));
    hierarchy_size_.push_back(hierarchy_size);

    for (int32_t j = 0; is_loaded && j < hierarchy_size; j++) {
      model_string.read(reinterpret_cast<char*>(&num_stage), sizeof(int32_t));
      num_stage_.push_back(num_stage);

      for (int32_t k = 0; is_loaded && k < num_stage; k++) {
        model_string.read(reinterpret_cast<char*>(&type_id), sizeof(int32_t));
        ...
        is_loaded = !model_string.fail() &&
          reader->Read(&model_string, classifier.get());
        if (is_loaded) {
      ...
      model_string.read(reinterpret_cast<char*>(&num_wnd_src), sizeof(int32_t));
      if (num_wnd_src > 0) {
        wnd_src_id_.back().resize(num_wnd_src);
        for (int32_t k = 0; k < num_wnd_src; k++) {
          model_string.read(reinterpret_cast<char*>(&(wnd_src_id_.back()[k])),
            sizeof(int32_t));
        }
      }
    }
